...
//			heading.setBackground(Color.black);
    ...
    public float getTotalPoints(double weight, int age, boolean sex) { //Passes values for use in calculation -R
        ...
        if (sex) {
            ...
            else if(totalCal > (((100*age) + 900)/3) & totalCal < (2 * (((100*age) + 900)/3))) {
                totalPoints += (-1 * (100 / (((100 * age) + 900) / 3))) * (totalCal - (2 * ((100 * age) + 900) / 3));
            } else if(totalCal >= (2 * (((100*age) + 900)/3))) {
                totalPoints += 0;
            } else {
                totalPoints += (100/(((100*age) + 900)/3)) * totalCal;
            }
        }
        else {
            if (totalCal <= 0 | totalCal > 2 * (((100*age) + 700)/3)) {
                totalPoints += 0;
            }
            else if(totalCal > (((100*age) + 700)/3)) {
                totalPoints += (-1 * (100/(((100*age) + 700)/3))) * (totalCal - (2*((100*age) + 700)/3));
            } else {
                totalPoints += (100/(((100*age) + 700)/3)) * totalCal;
            }
        }
        //SODIUM - 1500mg Daily
        ...
        if (totalSod <= 0 | totalSod > (2 * 500)) {
            ...
            totalPoints += (-1 * (100 / 500)) * (totalSod - 1000);
        }
        else {
            totalPoints += (100/500) * totalSod;
        }
        //SUGAR - 30g Daily
        ...
        if (totalSug <= 0 | totalSug > (2 * 10)) {
            ...
            totalPoints += (-1 * (100/10)) * (totalSug - 20);
        } else {
            totalPoints += (100/10) * totalSug;
        }
        /* Protein
            ...
            -R
         */
        if (totalPro <= 0 | totalPro > (2 * ((weight *.45)/3))) {
            totalPoints += 0;
        }
        else if(totalPro > ((weight *.45)/3)) {
            totalPoints += (-1 * (100/((weight *.45)/3))) * (totalPro - ((weight *.45)/3));
        } else {
            totalPoints += (100/((weight *.45)/3)) * totalPro;
        }
